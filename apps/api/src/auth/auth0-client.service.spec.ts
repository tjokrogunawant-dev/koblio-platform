...
describe('Auth0ClientService', () => {
  ...
  describe('authenticateByUsername', () => {
    it('should authenticate student via username (no email scope — COPPA)', async () => {
      mockFetch.mockResolvedValueOnce(
        jsonResponse(200, {
      ...
      const body = JSON.parse(options.body);
      expect(body.grant_type).toBe('password');
      expect(body.username).toBe('bobby1234');
    ...
    it('should throw UnauthorizedException on 401', async () => {
      ...
      await expect(
        service.authenticateByUsername('bobby1234', 'wrong-password'),
      ).rejects.toThrow(UnauthorizedException);
    });

    it('should throw UnauthorizedException on 403', async () => {
      mockFetch.mockResolvedValueOnce(
        jsonResponse(403, { error: 'unauthorized_client' }),
      );

      await expect(
        service.authenticateByUsername('bobby1234', 'password'),
      ).rejects.toThrow(UnauthorizedException);
    });
