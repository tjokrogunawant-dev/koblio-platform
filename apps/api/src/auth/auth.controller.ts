...
export class AuthController {
  ...
  async classCodeLogin(@Body() dto: ClassCodeLoginDto) {
    const { authResult, classroomId } =
      await this.authService.classCodeLogin(dto);
    return { ...authResult, classroom_id: classroomId };
  }

  @Get('class-code/:code/lookup')
  @Public()
  @ApiOperation({ summary: 'Look up classroom and student list by class code' })
  @ApiResponse({ status: 200, description: 'Classroom found' })
  @ApiResponse({ status: 404, description: 'Class code not found' })
  lookupClassCode(@Param('code') code: string) {
    return this.authService.lookupClassCode(code);
  }
